...
public class TestProtobufs {
  ...
  private static final AddressBook ab_ = Fixtures.buildAddressBookProto(false);
  private static final byte[] abBytes_ = ab_.toByteArray();

  private static final AddressBook abWithExt_ = Fixtures.buildAddressBookProto(true);
  private static final byte[] abWithExtBytes_ = abWithExt_.toByteArray();
  ...
  public void testDynamicParsing() {
    ...
    assertEquals(abWithExt_, Protobufs.parseDynamicFrom(AddressBook.class,
        Fixtures.buildExtensionRegistry(), abWithExtBytes_));
  }

  @Test
  public void testStaticParsing() {
    assertEquals(ab_, Protobufs.parseFrom(AddressBook.class, abBytes_));
    assertEquals(abWithExt_, Protobufs.parseFrom(AddressBook.class,
        Fixtures.buildExtensionRegistry(), abWithExtBytes_));
  }

  @Test
  public void testConverterParsing() {
    ProtobufConverter<AddressBook> protoConverter = ProtobufConverter.newInstance(
        AddressBook.class);
    assertEquals(ab_, protoConverter.fromBytes(abBytes_));

    ProtobufConverter<AddressBook> protoConverterWithExt = ProtobufConverter.newInstance(
        AddressBook.class, Fixtures.buildExtensionRegistry());
    assertEquals(abWithExt_, protoConverterWithExt.fromBytes(abWithExtBytes_));
